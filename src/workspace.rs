...
impl Workspace {
    ...
    /// Name from the cached identity cwd, without consulting live runtimes.
    /// Used by the tab-bar workspace label, which renders from `AppState` alone
    /// (the runtime registry is deliberately not part of it).
