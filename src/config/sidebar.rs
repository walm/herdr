...
pub enum SpaceSidebarToken {
    ...
    StateText,
    Workspace,
    /// Agent/CLI-reported marker of the most recently marked tab in the space.
    Marker,
    Branch,
    GitStatus,
    Custom(String),
...
fn space_token_name(token: &SpaceSidebarToken) -> String {
    match token {
        ...
        SpaceSidebarToken::Marker => "marker".into(),
...
mod tests {
    ...
    fn defaults_match_the_compact_agent_and_existing_space_layouts() {
        ...
        // The marker token is in our default space row so agent/CLI-reported
        // markers show without config, matching the tab bar.
        assert_eq!(
            config.spaces.rows,
            vec![
                vec![
