...
impl ClientShellState {
    ...
    pub(super) fn route_overlay_key(
        ...
        if matches!(self.overlay, Some(ClientShellOverlay::ConfirmClose(_))) {
            if key.code == KeyCode::Enter {
                ...
                self.push_endpoint_method(confirm.close_method(), outcome);
                outcome.repaint = true;
    ...
    /// Whether any pane in the workspace is pinned, so closing the workspace
    /// must ask even when `ui.confirm_close` is off.
    pub(super) fn workspace_has_pinned_pane(&self, workspace_id: &str) -> bool {
        self.snapshot.as_deref().is_some_and(|snapshot| {
            snapshot
                .panes
                .iter()
                .any(|pane| pane.workspace_id == workspace_id && pane.pinned)
        })
    }

    /// Ask before closing a pinned pane, or a tab that holds one. Accepting
    /// retries the same close with `force`.
    pub(super) fn open_confirm_pinned_close_overlay(&mut self, target: ClientConfirmCloseTarget) {
        let Some(snapshot) = self.snapshot.as_deref() else {
            return;
        };
        let (title, detail) = match &target {
            ClientConfirmCloseTarget::Pane { pane_id } => {
                ...
                ("Close pinned pane?".to_owned(), name)
            }
            ClientConfirmCloseTarget::Tab { tab_id } => {
                ...
                let pinned = snapshot
                    .panes
                    .iter()
                    .filter(|pane| &pane.tab_id == tab_id && pane.pinned)
                    .count();
                let panes = if pinned == 1 {
                    "1 pinned pane".to_owned()
                } else {
                    format!("{pinned} pinned panes")
                };
                (
                    "Close tab with pinned pane?".to_owned(),
                    format!("{} — {panes}", tab.label),
                )
            }
            ClientConfirmCloseTarget::Workspace { workspace_id } => {
                self.open_confirm_close_overlay(workspace_id.clone());
                return;
            }
        };
