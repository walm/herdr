...
/// What a close confirmation resolves to once accepted. Pinned panes and tabs
/// retry their own close with `force`; a workspace closes its whole group.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ClientConfirmCloseOverlay {
    /// The request that carries out the confirmed close.
    pub(super) fn close_method(self) -> crate::api::schema::Method {
        use crate::api::schema::{Method, PaneCloseParams, TabCloseParams, WorkspaceCloseParams};
        match self.target {
            ...
            ClientConfirmCloseTarget::Tab { tab_id } => Method::TabClose(TabCloseParams {
                tab_id,
                force: true,
            }),
            ClientConfirmCloseTarget::Pane { pane_id } => Method::PaneClose(PaneCloseParams {
                pane_id,
                force: true,
            }),
        }
...
pub(super) struct PendingEndpointRequest {
    ...
    /// Set when the request closes a pinned pane, or a tab holding one, without
    /// `force`. A `confirmation_required` reply then asks about that target
    /// instead of the workspace.
    pub(super) confirmation_pinned_target: Option<ClientConfirmCloseTarget>,
