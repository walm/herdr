...
impl ClientShellState {
                ...
                if action == crate::input::KeybindAction::CloseWorkspace {
                    if let Some(workspace_id) = self.workspace_action_id() {
                        if self.config.confirm_close
                            || self.workspace_has_pinned_pane(&workspace_id)
                        {
    ...
    pub(super) fn push_endpoint_method_with_kind(
        ...
        let confirmation_pinned_target = match &method {
            crate::api::schema::Method::TabClose(params) if !params.force => snapshot
                .panes
                .iter()
                .any(|pane| pane.tab_id == params.tab_id && pane.pinned)
                .then(|| ClientConfirmCloseTarget::Tab {
                    tab_id: params.tab_id.clone(),
                }),
            crate::api::schema::Method::PaneClose(params) if !params.force => snapshot
                .panes
                .iter()
                .any(|pane| pane.pane_id == params.pane_id && pane.pinned)
                .then(|| ClientConfirmCloseTarget::Pane {
        ...
        let repaint = match result {
            ...
            Err(error)
                if error.code.as_deref() == Some("confirmation_required")
                    && pending.confirmation_pinned_target.is_some() =>
            {
                // Pinning asks regardless of `ui.confirm_close`: the pin is an
                // explicit per-pane request.
                if let Some(target) = pending.confirmation_pinned_target {
                    self.open_confirm_pinned_close_overlay(target);
                }
                true
            }
            Err(error)
    ...
    pub(super) fn endpoint_method_for_action(
        ...
        use crate::api::schema::{
            ...
            SplitDirection, TabCloseParams, TabCreateParams, TabMoveParams, TabTarget,
            WorkspaceTarget,
        };
        ...
        match action {
            ...
            KeybindAction::CloseTab => Some(Method::TabClose(TabCloseParams {
                tab_id: focused_tab?,
                force: false,
            })),
            KeybindAction::TogglePinPane => {
                let pane_id = focused_pane.clone()?;
                let pinned = snapshot
                    .panes
                    .iter()
                    .any(|pane| pane.pane_id == pane_id && pane.pinned);
                Some(Method::PaneSetPinned(
