...
fn pinned_pane_close_asks_then_retries_with_force() {
    ...
    let [ClientShellAction::Endpoint { request, .. }] = &close.actions[..] else {
        panic!("close pane should use endpoint API");
    };
    assert!(matches!(
        &request.method,
        crate::api::schema::Method::PaneClose(params)
    ...
    let (repaint, actions) = state.handle_endpoint_result(
        ...
        Err(ClientShellEndpointError {
            ...
            message: "pane is pinned; pass force to close it".into(),
        }),
    );
    assert!(repaint);
    ...
    assert!(
        ...
        "a pin asks even with confirm_close off"
    );

    let confirm = state.handle_input_bytes(b"\r");
    let [ClientShellAction::Endpoint { request, .. }] = &confirm.actions[..] else {
        panic!("pinned confirmation should retry through the endpoint API");
    };
    assert!(matches!(
        &request.method,
        crate::api::schema::Method::PaneClose(params)
...
#[test]
fn tab_with_pinned_pane_close_asks_about_the_tab_not_the_workspace() {
    let mut snapshot = snapshot();
    snapshot.panes[0].pinned = true;
    let mut state = ClientShellState::new(ClientShellConfig::from_config(&Config::default()));
    state.set_snapshot(Box::new(snapshot));
    ...
    let [ClientShellAction::Endpoint { request, .. }] = &close.actions[..] else {
        panic!("close tab should use endpoint API");
    };
    let request_id = request.id.clone();
    state.handle_endpoint_result(
        ...
        Err(ClientShellEndpointError {
            code: Some("confirmation_required".into()),
            message: "tab contains a pinned pane".into(),
        }),
    );
    assert!(matches!(
        state.overlay.as_ref(),
    ...
    let confirm = state.handle_input_bytes(b"\r");
    let [ClientShellAction::Endpoint { request, .. }] = &confirm.actions[..] else {
        panic!("tab confirmation should retry through the endpoint API");
    };
    assert!(matches!(
        &request.method,
        crate::api::schema::Method::TabClose(params) if params.tab_id == "tab_1" && params.force
    ));
}
