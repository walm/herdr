...
fn workspace_row_height(app: &AppState, ws: &crate::workspace::Workspace, indented: bool) -> u16 {
    ...
    let marker = app
        .tab_markers
        .then(|| ws.aggregate_marker(&app.terminals))
        .flatten()
        .map(|(marker, _)| marker);
    tokens::space_rows(
        &app.sidebar_spaces,
...
fn resolved_token_spans(
    ...
    for (position, index) in visible_indices.iter().copied().enumerate() {
        ...
        match &token.kind {
            ...
            ResolvedTokenKind::Marker(marker) => {
                spans.push(Span::styled(
                    truncate_end(marker, budgets[index]),
                    apply_token_style(Style::default().fg(p.text), token.style),
                ));
            }
            ResolvedTokenKind::StateText(text) => {
                spans.push(Span::styled(
