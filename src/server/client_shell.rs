...
pub(super) fn snapshot(
    ...
    let panes = snapshot
        ...
        .map(|pane| {
            ...
            let pinned = pane_state.is_some_and(|pane| pane.pinned);
...
mod tests {
    ...
    #[test]
    fn snapshot_carries_pinned_panes_and_markers_to_the_client() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = crate::app::App::new(
            &crate::config::Config::default(),
            crate::app::AppPolicy::TEST,
        ...
        assert!(!before.panes[0].pinned);
        assert_eq!(before.tabs[0].marker, None);
        assert_eq!(before.workspaces[0].marker, None);
