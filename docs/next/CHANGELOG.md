...
- `--help` and `-h` are now generated from the CLI spec for every command and subcommand, so each flag carries a description and `--help` adds a copy-pasteable EXAMPLES block (`-h` stays terse). This replaces the hand-written usage strings, which could drift from the spec that drives shell completions. Adding `herdr terminal session control` to the spec along the way also fixed it missing from completions.
- Added agent/CLI-settable tab markers: `herdr pane report-metadata <pane> --marker "🔨"` (cleared with `--clear-marker`) shows a short emoji/icon in a fixed two-column slot before the tab's agent status glyph, and on the sidebar workspace row after the color swatch. The most recently marked pane wins for a tab, and the most recently marked tab wins for a workspace. Markers are display-only, honour `--ttl-ms`, and can be hidden with `ui.tab_markers = false`.
