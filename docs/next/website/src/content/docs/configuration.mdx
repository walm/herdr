...
`last_tab` (default `prefix+a`) switches back to the previously active tab within the current workspace, toggling between the two like `last_pane` does for panes. It is scoped per workspace.
...
The symbols are static. For an animated working indicator, choose **animated spinner** in Settings or configure `status_indicators = "spinner"`: it uses the distinct symbols for blocked, done, idle, and unknown, and a braille spinner while an agent works. The animation runs only on the client you are looking at and only while a working agent is visible, so it costs the server nothing and stops on its own once agents go quiet.
...
Set `tab_markers = false` to hide agent-reported markers. Agents and scripts can set a short marker (an emoji or icon) on their pane with `herdr pane report-metadata "$HERDR_PANE_ID" --source build --marker "🔨"`, and clear it with `--clear-marker`. The marker appears after the tab label in the tab bar, and as the `marker` token on sidebar workspace rows (in the default row, after the workspace name). When several panes in a tab (or several tabs in a workspace) have markers, the most recently reported one wins. Markers are display-only, are capped at two columns, support `--ttl-ms` to expire on their own, and are cleared on restart. Nothing is shown unless a marker is reported, so this is on by default.

Pinned panes ask before closing. Pin from the pane right-click menu, with `herdr pane pin <pane>`, or with the optional `keys.toggle_pin_pane` binding. Closing a pinned pane, a tab that holds one, or a workspace that holds one shows a confirmation even when `confirm_close = false`; `herdr pane close --force` and `herdr tab close --force` skip it. Pins persist across restarts and list with `herdr pane list --pinned`.

Set `show_prefix_hint = false` to hide the hint bar that appears at the bottom of the screen after you press the prefix key. Prefix mode still works; only the hint is hidden.

Set `tab_number_prefix = true` to prefix each named tab in the tab bar with its position number, for example `1: main`, so the `prefix+1..9` switch keys are easy to read. Unnamed tabs already show their number and are left alone.

Set `tab_agent_status = true` to show each tab's aggregate agent status at the end of the tab in the tab bar, using the same indicator style as the sidebar. Only working, blocked, and done tabs get a glyph; quiet tabs stay clean.

`workspace_tab_label` shows the active workspace's name at the right of the tab bar so you can tell which workspace you are in while the sidebar is out of the way. `auto` (default) shows it only when the sidebar is collapsed, `on` always, `off` never. Give a workspace its own color by right-clicking it in the sidebar and choosing "Set color", or with `herdr workspace set-color <workspace> teal`; the label takes that color. Colors persist across restarts.

`agent_panel_scope` controls which agents the sidebar panel lists. `all` (default) shows agents across every workspace and machine. `current` shows only the agents in the current workspace: the highlighted one while you navigate the sidebar, otherwise the focused one. Next/previous agent keys follow the same scope. Click the `all`/`current` toggle in the agents header to switch at runtime; that choice is remembered per client and wins over the config value until you change it again.
