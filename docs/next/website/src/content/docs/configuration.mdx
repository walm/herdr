...
`last_pane_in_tab` is the same idea scoped to the current tab: it toggles between the two most recently focused panes within the active tab, like tmux's `last-pane`. Each tab tracks its own previous pane, and it never jumps to another tab or workspace. Unset by default.

`last_tab` switches back to the previously active tab within the current workspace, toggling between the two like tmux's last-window. Each workspace remembers its own previous tab. It defaults to `prefix+a` ("alternate" tab); set it to an empty string to disable.

By default, pressing the prefix key twice (for example `ctrl+b` `ctrl+b`) sends a single literal prefix keystroke to the focused pane, matching tmux's send-prefix. You can override that by binding an action to the prefix key itself, e.g. `last_tab = "prefix+ctrl+b"` when the prefix is `ctrl+b`. The bound action then runs on a double-tap and send-prefix is no longer available for that key, so only override it if you do not rely on sending a literal prefix into pane apps such as a nested tmux.
...
`agent_panel_scope` controls which agents the panel lists. `all` (default) shows agents across every workspace. `current` shows only agents in the active workspace, so the list follows the workspace you switch to (and the highlighted workspace while you browse the sidebar). Click the scope label in the panel header to toggle it, or set it here. It is independent of `agent_panel_sort`.

`workspace_tab_label` shows the active workspace's name at the right of the tab bar so you can tell which workspace you're in when the sidebar isn't showing it. `auto` (default) shows it only while the sidebar is collapsed, `on` always shows it, and `off` hides it. You can tint the label per workspace: right-click a workspace in the sidebar, choose **Set color**, and pick one of the theme colors (or `none` to clear). The color is saved with your session and picks a readable text color automatically.
...
Set `show_prefix_hint = false` to hide the prefix hint bar that appears at the bottom of the screen after you press the prefix key. Prefix mode still works; only the hint is hidden.

Set `tab_number_prefix = true` to prefix each tab label in the tab bar with its position number, for example `1: main`. This makes the `prefix+1..9` switch keys easy to read on named tabs. Unnamed tabs already display their number, so they are left unchanged.

Set `tab_agent_status = true` to show each tab's aggregate agent status at the end of the tab in the tab bar, using the same glyphs as the sidebar: an animated spinner for working, a red dot for blocked, and a teal dot for done. Quiet tabs (idle or without an agent) show nothing. When a tab has several agents, the highest-attention one wins (blocked, then done, then working). This makes it easy to see which tabs need attention when switching workspaces.

Set `tab_markers = false` to hide agent-reported markers. Agents and scripts can set a short marker (an emoji or icon) on their pane with `herdr pane report-metadata "$HERDR_PANE_ID" --source build --marker "🔨"`, and clear it with `--clear-marker`. The marker appears in a fixed two-column slot just before the tab's agent status glyph, and on the workspace row in the expanded sidebar just after the color swatch. When several panes in a tab (or several tabs in a workspace) have markers, the most recently reported one wins. Markers are display-only, are capped at two columns, support `--ttl-ms` to expire on their own, and are cleared on restart. Nothing is shown unless a marker is reported, so this is on by default.
